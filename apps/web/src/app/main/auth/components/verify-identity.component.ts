...
export class VerifyIdentityComponent {
	...
	@Output() public accessData: EventEmitter<string | ResetPassword>
		= new EventEmitter<string | ResetPassword>();
	...
	public verifyOTP() {
		...
		const data: IVerifyData = {
			...
			otp: this._getOTP() };
		const service: Observable<IVerifySignUp | ResetPassword>
		...
		.subscribe({
			next: ( accessData: IVerifySignUp | ResetPassword ) => {
				if ( !accessData ) {
